...
class Autoencoder {
  ...
  sigmoid(z) {
    var scope = {
      z: typeof z === "number" ? this.MathJS.matrix([[z]]) : z
    },
        sigmoid;
    if (scope.z.size()[1] === undefined) scope.ones = this.MathJS.ones(scope.z.size()[0]);else scope.ones = this.MathJS.ones(scope.z.size()[0], scope.z.size()[1]);
  ...
  costFunction(X, Y, algorithm_mode) {
    ...
    if (this.optimization_mode.mode === 1) {
      //cost for mini-batch gradient descent.
      ...
      if (this.algorithm_mode === 0) J = this.MathJS.sum(this.MathJS.eval('0.5*((y-y_result).^2)', scope)) / this.optimization_mode.batch_size + this.regularization_param / 2 * (this.MathJS.sum(this.MathJS.eval('W1.^2', scope)) + this.MathJS.sum(this.MathJS.eval('W2.^2', scope))); //cost with regularization parameter.
      else if (this.algorithm_mode === 1 || this.algorithm_mode === 2) J = this.MathJS.sum(this.MathJS.eval('0.5*((y-y_result).^2)', scope)) / this.optimization_mode.batch_size;
    } else if (this.optimization_mode.mode === 2 && iteration_count <= scope.x.size()[0]) {
      // cost for stochastic gradient descent.
      ...
      if (this.algorithm_mode === 0) J = this.MathJS.sum(this.MathJS.eval('0.5*((y-y_result).^2)', scope)) + this.regularization_param / 2 * (this.MathJS.sum(this.MathJS.eval('W1.^2', scope)) + this.MathJS.sum(this.MathJS.eval('W2.^2', scope))); //cost with regularization parameter.
      else if (this.algorithm_mode === 1 || this.algorithm_mode === 2) J = this.MathJS.sum(this.MathJS.eval('0.5*((y-y_result).^2)', scope));
    } else if (this.optimization_mode.mode === 0) {
      //cost with batch gradient descent.
      this.y_result = this.forwardPropagation(scope.x, undefined, undefined);
      scope.y_result = this.y_result;

      if (this.algorithm_mode === 0) J = this.MathJS.sum(this.MathJS.eval('0.5*((y-y_result).^2)', scope)) / scope.x.size()[0] + this.regularization_param / 2 * (this.MathJS.sum(this.MathJS.eval('W1.^2', scope)) + this.MathJS.sum(this.MathJS.eval('W2.^2', scope))); //cost with regularization parameter.
      else if (this.algorithm_mode === 1 || this.algorithm_mode === 2) J = this.MathJS.sum(this.MathJS.eval('0.5*((y-y_result).^2)', scope)) / scope.x.size()[0];
    }
  ...
  saveWeights(weights, path) {
    ...
    if (Object.keys(window).length === 0) {
      ...
      localStorage['weights'] = JSON.stringify(this.W);
    }
  ...
  gradientDescent(X, Y, W1, W2) {
    ...
    if (this.algorithm_mode == 0) console.log('Training ...\n');

    while (true) {

      if (x !== undefined && y !== undefined && W1 !== undefined && W2 !== undefined) gradient = this.costFunction_Derivative(x, y, W1, W2);else gradient = this.costFunction_Derivative(undefined, undefined, undefined, undefined);
      scope.W1 = W1 || this.W1;
      scope.W2 = W2 || this.W2;
      ...
      if (x !== undefined && y !== undefined) cost = this.costFunction(x, y, undefined);
  ...
  train_network(X, Y) {
    ...
    if (this.y.size()[0] !== this.x.size()[0]) {
      ...
      if (this.hiddenLayerSize !== undefined) {
        if (this.hiddenLayerSize >= this.x.size()[1] + 1) this.hiddenLayerSize = this.hiddenLayerSize;else this.hiddenLayerSize = this.x.size()[1] + 1;
      } else if (this.hiddenLayerSize === undefined) this.hiddenLayerSize = this.x.size()[1] + 1;
  ...
  setWeights(path) {
    ...
    if (Object.keys(window).length === 0) {
      ...
      this.W = JSON.parse(localStorage['weights']);
    }
  }
}
